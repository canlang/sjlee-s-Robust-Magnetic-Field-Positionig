clear; close all;
% purpose: convert to iphone dataset
...
sample_freq = 60;
minPeakHeight = std(rawdata.acc_norm);       
minPeakHeight = minPeakHeight*.8;
% [pks,locs] = findpeaks(rawdata.acc_norm,'MinPeakDistance',...
%     .3/rate,'MinPeakHeight',minPeakHeight);   % .3s 이내의 피크는 무시 (가정: 발걸음이 .3초 내에는 2번이뤄지지 않음)\
[pks,locs] = findpeaks(rawdata.acc_norm,'MinPeakHeight',minPeakHeight...
    ,'MinPeakDistance',.3/sample_freq);
...
euler = quatern2euler(quaternConj(rawdata.quaternion(locs,:)));	
% euler = rawdata.euler(locs,:);
% euler(:,3) = euler(:,3)-euler(1,3);
% euler = quatern2euler(quaternConj(quaternion)) * (180/pi);
d_heading = [0;diff(euler(:,3))];

% rotMat = euler2rotMat(euler);
rotMat = quatern2rotMat(rawdata.quaternion(locs,:));
% rotMat = quatern2rotMat(quaternConj(rawdata.quaternion(locs,:)));
...
%%
% close all
% euler1 = rawdata.euler(locs,:);
% subplot(311)
% plot(euler1)
% 
% euler2 = quatern2euler((rawdata.quaternion(locs,:)));	
% subplot(312)
% plot(euler2)
% 
% euler3 = quatern2euler(quaternConj(rawdata.quaternion(locs,:)));	
% subplot(313)
% plot(euler3)
...
% layout = jsondecode(fileread('map/N1-7F-.json'));
...
switch site_name
    case 'N1-7F'
        xWorldLimits = [-1 (size(A,2)-20)*scale];
        yWorldLimits = [-1 (size(A,1)-20)*scale];
...
% ps.x = 34+random('normal',0,1,n,1);
% ps.y = 9+random('normal',0,1,n,1);
